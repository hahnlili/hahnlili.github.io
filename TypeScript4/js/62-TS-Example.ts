...
interface Monster {
    ...
    monsterFrisur: string; //Die Frisur vom Monster
    monsterLieblingsGetränk: string;
    monsterImgPfad: string;
}
...
let playerName: string = "Marleen";
let playerLevel: number = 1;                                            // Ein paar globale Variablen, welche den Spieler darstellen.
...
let monsterName: string[] = ["Bobbi", "boss", "Donald", "Justin", "Pika", "putin"]; // length = 3, da 3 Einträge. Von 0-2.
...
let monsterModifers: string[] = ["Ist nerviiiig", "Linkshänder", "Bier-Connoisseur", "Verfehlt häufig", "Prokrastiniert", "Müde", "Verwirrt", "Wasserscheu", "Bipolar", "Hat Schnupfen", "Verläuft sich oft"]; // Eine Reihe von zufälligen "Verstärkern" für das Monster.

// -- Initialisierung für viele/variable Anzahl an Monster --
let monsterArray: Monster[] = []; // Das Haupt-Array wurde erstellt und initialisiert!
console.log(monsterArray); // Gebe das Monster-Array einmal zu beginn aus. Es sollte leer sein.

let frisur: string[] = ["Pilzkopf", "Fokuhila", "Topfschnitt", "Irokese", "Feuer"];

let getränk: string[] = ["Wodka", "Tequilla", "Apfelsaft", "Benzin", "GinTonic"];

let kartoffelchip: number[] = [];
...
window.onload = function () {
    ...
    console.log(document.getElementById("monsterSpawner").innerHTML); //war außerhalb des onloadWindow, wurde aufgerufen bevor seite geladen war. 
    neuesArray(10);
    neuesArray(14);
    neuesArray(7);
    neuesArray(130);
    neuesArray(107);
    document.getElementById("monsterFighter").addEventListener("click", fightAllMonsters, false);
    document.getElementById("monsterWeakFighter").addEventListener("click", fightAllWeakMonsters, false);
    document.getElementById("monsterWeakestFighter").addEventListener("click", fightWeakestMonster, false);
}

function neuesArray(zahl: number) {

    kartoffelchip.push(zahl);
    console.log(kartoffelchip);
...
function generateMonster() {
    ...
    for (let i: number = 0; i < rndNumb; i++) {
        ...
        let newMonsterLvl: number = generateMonsterLevel();
        let newMonsterModifier: string[] = generateMonsterModifer();       // Eigens-gebaute Funktion, welche ein string-Array zurück gibt.
        let newMonsterFrisur: string = generateMonsterFrisur();
        let newMonsterLieblingsgetränk: string = generateMonsterLieblingsGetränk();
        let newMonsterImgPfad: string = generateMonsterImgPfad();

        let newMonster: Monster = {                                        // Monster wird erstellt.
            monsterName: newMonsterName,
            monsterHealthPoints: newMonsterHP,
            monsterExperience: newMonsterXP,
            monsterLevel: newMonsterLvl,
            monsterModifier: newMonsterModifier,
            monsterFrisur: newMonsterFrisur,
            monsterLieblingsGetränk: newMonsterLieblingsgetränk,
            monsterImgPfad: newMonsterImgPfad
            // monsterMoney war nicht im Monster enthalten. 
...
function generateMonsterImgPfad(): string {
    let newMonsterImgPfad: string = "";
    let rndNumber: number = getRNGNumber(monsterName.length);
    newMonsterImgPfad = "imgs/" + monsterName[rndNumber] + ".jpg";
    return newMonsterImgPfad;
}


function generateMonsterFrisur(): string {
    let newMonsterFrisur: string = "";
    let rngNumber: number = getRNGNumber(frisur.length);
    newMonsterFrisur = frisur[rngNumber];
    return "Frisur: " + newMonsterFrisur;

}

function generateMonsterLieblingsGetränk(): string {
    let newMonsterLieblingsgetränk: string = "";
    let rngNumber: number = getRNGNumber(getränk.length);
    newMonsterLieblingsgetränk = getränk[rngNumber];
    return "Trinkt gern:  " + newMonsterLieblingsgetränk;
...
function getRNGNumber(_maxNumber: number): number {
    ...
    //return Math.floor(Math.random() * _maxNumber);    --> optionale aufgabe	                                                           // Diese Zeile ist einer der drei Fehler in den Funktionen. Ich bin mal so frei und vermerke das hier. Einfach löschen und alles wird besser.
    return rngNumber;                                                   // return war = 0, ergibt keinen sinn, soll ja random sein. Gebe diese Zahl zurück, Funktion kann ähnlich einer Variable in Rechnungen genutzt werden.
}
...
function generateMonsterName(): string {
    ...
    generatedMonsterName += monsterName[rngNumber];    //monsterName war [0], ergibt keinen sinn, man will ja ne zufällige Zahl                         // Füge den Monsternamen zusammen: nimm aus dem entsprechenden Array mit der zufallsgenerierten Zahl den entsprechenden Eintrag.
...
function generateMonsterLevel(): number {
    // Diese Funktion gibt eine zufällige ganze Zahl (zwischen 0 und 350) + 100 zurück.
    let tempMonsterLvl: number = 0 + getRNGNumber(11);
    return tempMonsterLvl;
...
function fightAllWeakMonsters() {
    for (let i: number = monsterArray.length; i > 0; i--) {
        if (monsterArray[i - 1].monsterLevel < playerLevel) {
            fightMonster(i);
        }
    }
}
function fightWeakestMonster() {
    console.log("ich finde jetzt das schwächste!!!");
    let weakest: number = monsterArray.length - 1;
    for (let i: number = monsterArray.length; i > 0; i--) {
        if (monsterArray[i - 1].monsterLevel < monsterArray[weakest].monsterLevel) {
            weakest = i - 1;

        }
    }
    console.log("unser schwächstes Monster hat level: " + monsterArray[weakest].monsterLevel);
    console.log("spielerlevel ist: " + playerLevel);
    if (monsterArray[weakest].monsterLevel < playerLevel) {
        fightMonster(weakest + 1);
    }
}
...
function fightMonster(_index: number) {
    ...
    if (monsterArray[_index-1].monsterLevel < playerLevel) {
        updateHTML();
...
function updatePlayerLevel(differenz: number) {
    if(playerXP + differenz >= 0){
    playerXP += differenz;                 	    // _index ist in diesem Fall die Länge des Arrays - allerdings zählt der Computer beginnend von null, nicht eins! Deshalb _index-1.
    playerLevel = (Math.floor(playerXP / playerXPperLevel))+1;                                                                           // Spieler-Level = XP / XPproLevel
    document.getElementById("xpCounter").innerHTML = "Player-Level: " + playerLevel + " (XP: " + playerXP + " / " + ((playerLevel + 1) * playerXPperLevel) + ")";       // Baue den String für die Spieler-Info zusammen
