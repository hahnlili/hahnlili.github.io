...
function generateCards (){
    let tempArray: card[]=[];
    for ( let i=1; i <= 10; i++){
        let newBlueCard: card = {
            value: i, 
            colour: "blue",
            covered: true
        };
        let newGreenCard: card = {
            value: i, 
            colour: "green",
            covered: true
        };
...
function mixCards (deck: card[]){

    let temp :card[] = [];
    while (deck.length != 0){
        let rndNumber: number = Math.floor(Math.random()*deck.length);
...
function handOut (deck: card[], count:number, covered: boolean){
