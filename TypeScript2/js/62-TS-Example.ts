...
function monsterGenerateHTML()
{
    ...
    monsterFrisur.innerHTML = monsterArray[monsterArray.length -1].monsterFrisur;
    holdingDiv.appendChild(monsterFrisur);

    let monsterLieblingsGetränk : HTMLElement = document.createElement("p");
    monsterLieblingsGetränk.innerHTML = monsterArray[monsterArray.length -1].monsterLieblingsGetränk;
    ...
    monsterImg.setAttribute("src", monsterArray[monsterArray.length - 1].monsterImgPfad);                 // Der Pfad für das Bild muss über setAttribute festgelegt werden. Der Bildpfad kann natürlich auch anders aussehen.
    console.log(monsterArray[monsterArray.length - 1].monsterImgPfad);
